...
@section('title','Tambah Week Template')
@section('content')
<h3>Tambah Week Template</h3>
<div class="card p-4">
  <form method="POST" action="{{ route('admin.week-templates.store') }}">@csrf
    <div class="row">
      <div class="col-md-6">
        <div class="mb-3">
      ...
      <div class="col-md-6">
        <div class="mb-3">
          ...
          <select name="week_type" class="form-control @error('week_type') is-invalid @enderror" required>
            ...
            <option value="1" {{ old('week_type') == '1' ? 'selected' : '' }}>Minggu 1 (Kejuruan)</option>
            <option value="2" {{ old('week_type') == '2' ? 'selected' : '' }}>Minggu 2 (Umum)</option>
          </select>
    ...
    <div class="mb-3">
      <h5>Pilih Day Template untuk Setiap Hari</h5>
      ...
      <div class="mb-2 p-3 border rounded">
        ...
        <select name="days[{{ $idx }}][day_template_id]" class="form-control">
          ...
          @foreach($dayTemplates as $dt)
            <option value="{{ $dt->id }}">{{ $dt->name }} ({{ $dt->slots->count() }} slots)</option>
          @endforeach
        </select>
